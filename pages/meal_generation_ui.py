...
class MealGenerator(ft.Container):
    ...
    def meal_Type(meal_num,page,table,max_calories):
        ...
        def submit():
            ...
            p1 = MealGenerator.meals_generation2(checkbox_states,max_calories)
            MealGenerator.addrows(p1,table)
            page.add(submission_row)
            page.update()
...
def ui(page: ft.Page):
    ...
    view, table = MealGenerator.create_datatables() 



    page.add(
        ft.Column(expand=True,scroll=ft.ScrollMode.ALWAYS, controls=[
            MealGenerator.information(1000, 3), #meal calories, mealnum
            *MealGenerator.meal_Type(3,page,table,1000), #mealnum, calories, page
            view
        ])
    )
